...
???
I hope that if you are here at this talk, you already know 
the value of software testing.
And more importantly, the value of a test suite that's so robust
that you trust your changes and have fun writing code 
and taking risks, knowing the safety net of tests will 
help give you freedom.

The part we're focusing on today is how to have a complete
set of test cases that's easy to write and maintain.
...
This is in place for the examples.
...
???
* The `pytest.ini` file is in use for my examples.
* If you don't know what this is, it's not important for this
  talk, but it affects the output, so I thought I'd mention it
* We don't really care about the traceback right now, so
  I've turned it off.
* I also want to show the test names, thus verbose.
* And we'll use this marker later on in the talk.
---
...
* I had to look it up.
* This has at least one bug in it.
...
```
???
* Since there are 4 possible outcomes, 
we need at least 4 test cases.
* we need more test cases and it's already kinda ugly
    * boundary values
    * negative numbers
    * floats
...
Problems:
* redundancy
* if tests more complicated, even more pain
* new test cases often involves cut/paste/modify, 
...
However
* there's really 4 test cases, pytest shows 1
...
* the list of parameter rows can be in-line, 
---
# Function Parametrization
`test_3.py`:
```python
...
```
???
* When we run it we see all the test cases
...
???
I find it easier to read if I name the list.
...
I also changed the name to test_func because the other two ways
However, since function parametrization should be your goto method
for lots and lots of tests, let's look at some tricks with running
parametrized tests.
---
...
???
* Basic syntax
* I like to do fixture parametrization returning a row of related
values
* but you can parametrize a single param fixture as well, and then
  you wouldn't have to unpack it
---
# Fixture Parametrization
`test_8.py:`
```python
...
```
???
* problem is the identifier, yuk, let's fix it before moving on
...
```
???
or a function that can take one of the parametrizations and return a string
...
Fixture parametriziton is great 
...
```
???
The third technique is using the hook function pytest_generate_tests
I reserve this for use cases that I can't figure out with function or fixture
parametrization. But honestly, I usually end up refactoring back to a more 
standard parametrization method.
Benefits:
- generate parametrizations based on more info, metafunc has a lot of introspection
- you can dynamically generate params based on lots of introspection options
